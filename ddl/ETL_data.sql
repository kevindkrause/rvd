...
begin
	...
	begin try
        ...
        update dbo.volunteer
		set
			App_Pursued_By_Value = null,
			update_date = getdate()
        where App_Pursued_By_Value is not null
...
begin
	...
	begin try
        ...
        -- DEACTIVATE PURSUIT IF VOL_DESK_USER_KEY IS NOT SET
        update dbo.Volunteer_Pursuit_Hist
        set active_flag = 'N'
        where active_flag = 'Y'
            and exists ( select 1 from volunteer v where v.volunteer_key = volunteer_pursuit_hist.volunteer_key and v.Vol_Desk_User_Key = 1 )
...
/***********************************************************
**				   DEPT ROLE STATUS UDPATE
***********************************************************/
if object_id('dbo.ETL_Status_Update_Process_Roles') is null
    exec( 'create procedure dbo.ETL_Status_Update_Process_Roles as set nocount on;' )
go

alter procedure dbo.ETL_Status_Update_Process_Roles
as
begin
	set nocount on
